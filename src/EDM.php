...
class EDM
{
    ...
    public function GetInvoiceRequest($faturaNo, $contentType = 'XML', $gelen = false)
    {
        $req_header = new RequestHeader();
        $req_header->session_id = $this->getSessionId();
        $send_data = $req_header->getArray();
    ...
    public function GetInvoiceStatus($faturaNo, $faturaUUID)
    {
        $req_header = new RequestHeader();
        $req_header->session_id = session('EFATURA_SESSION');
        $send_data = $req_header->getArray();
        $send_data['INVOICE'] = ['_' => '', 'ID' => $faturaNo, 'UUID' => $faturaUUID];
        $req = new Request();
